...
# pandas imported conditionally in extract_text_from_csv
...
def extract_text_from_csv(file_path, document_name=None, batch_size=100):
    """
    Extract text from CSV file by converting rows to readable text.
    Processes ALL rows and columns in batches to avoid overwhelming the pipeline.
    ...
    Returns:
        Generator of text chunks, each containing batch_size rows (or fewer for last batch)
    """
    try:
        try:
            import pandas as pd
            ...
            yield extract_text_from_csv_basic(file_path)
            return
        ...
        # First batch: header and metadata
        text_lines = []
        text_lines.append(f"CSV Dataset: {total_rows} rows, {total_cols} columns")
        ...
        for start_idx in range(0, total_rows, batch_size):
            ...
            # Add batch header
            if start_idx == 0:
                text_lines.append(f"Rows {start_idx + 1} to {end_idx} of {total_rows}:")
            else:
                text_lines.append(f"\nRows {start_idx + 1} to {end_idx} of {total_rows}:")
            
            # Process all columns for each row in this batch
            for idx, row in batch_df.iterrows():
                row_items = list(row.items())
                row_text = " | ".join([f"{col}: {val}" for col, val in row_items])
...
def extract_text_from_csv_basic(file_path):
    """Basic CSV reading without pandas - fallback method"""
    try:
        import csv
        ...
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            if header:
                ...
                # Limit rows based on number of columns
                num_cols = len(header)
                if num_cols > 20:
                    max_rows = 20  # Very wide tables: only 20 rows
                elif num_cols > 10:
                    max_rows = 50  # Wide tables: 50 rows
                else:
                    max_rows = min(100, total_rows)  # Normal tables: up to 100 rows
                
                # Limit columns per row
                max_cols_per_row = 15
                
                text_lines.append(f"CSV Dataset: {total_rows} rows, {len(header)} columns")
                text_lines.append(f"Columns: {', '.join(header)}")
                text_lines.append("")
                text_lines.append("Sample data:")
                
                # Add sample rows with column limiting
                for row in all_rows[:max_rows]:
                    if len(row) > max_cols_per_row:
                        # Show first few and last few columns
                        shown_cols = row[:max_cols_per_row//2] + row[-max_cols_per_row//2:]
                        shown_header = header[:max_cols_per_row//2] + header[-max_cols_per_row//2:]
                        row_text = " | ".join([f"{shown_header[i] if i < len(shown_header) else f'Col{i+1}'}: {val}" for i, val in enumerate(shown_cols)])
                        row_text += f" | ... ({len(row) - max_cols_per_row} more columns)"
                    else:
                        row_text = " | ".join([f"{header[i] if i < len(header) else f'Col{i+1}'}: {val}" for i, val in enumerate(row)])
                    text_lines.append(row_text)
                    row_count += 1
                
                if total_rows > max_rows:
                    text_lines.append(f"... ({total_rows - max_rows} more rows)")
        
        return "\n".join(text_lines) if text_lines else f"CSV file: {file_path}"
    except Exception as e:
