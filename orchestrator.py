...
def find_agents_for_query(query: str, query_type: str, attribute: Optional[str] = None, query_info: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if query_type == "filter" and attribute:
        ...
        if employee_name:
            ...
            if matching_agents:
                ...
                routing_info["reason"] = f"Employee '{employee_name}' not found in tracked agents, querying all"
        else:
            routing_info["strategy"] = "all_agents"
            routing_info["reason"] = "Could not extract employee name from query"
    
    # For min/max queries, need to query all agents to find global min/max
    elif query_type == "structured" and query_type in ["max", "min"]:
        routing_info["strategy"] = "all_agents"
        routing_info["reason"] = f"{query_type.upper()} query requires checking all agents for global comparison"
    ...
    elif "statistic" in query_lower or "average" in query_lower or "mean" in query_lower or "correlation" in query_lower:
...
def orchestrate_query(query: str, query_info: Dict[str, Any]) -> Tuple[Optional[str], List[Dict[str, Any]], Dict[str, Any]]:
    ...
    try:
        ...
        if routing_info["strategy"] == "statistics_agent":
            # Delegate to statistics agent
            orchestrator.status = "active"
            return None, [], routing_info  # Let LLM handle with statistics context
